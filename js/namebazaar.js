...
$(window).resize(function(){
	...
	// adjust image to fit vertical space
	bH = $('#background .base').height();
	curSc = Number($('#background').attr('data-sc'));

	ratio = .62;
	trgR = ratio*winH;
	trgSc = trgR/bH;	
	curRat = bH/winH;
	//console.log(bH+' / '+curRat+' / '+trgSc)
	
	breakH = 850;
	if(winW>1200){breakH = 1000;}
	if(winW<=750){breakH = 900;}
	if(winH<breakH){
		offset = .95;
		if(winW<=750){offset = .85;}
		if(winW<=550){offset = .84;}
		newS = offset-((origH/winH) - ratio)
		newM = winH - ((origM*newS)+(origH*newS))
		if(winW<=414){newM+=-5}
		
		if(newS>.2){
			$('#background').show();
			$('#background').attr('data-sc',newS);
			$('#background').css({'transform':'scale('+newS+')','top':newM+'px'})
		} else {
			$('#background').hide();
		}
		
	} else {
		$('#background').attr('data-sc',1);
		$('#background').css({'transform':'','top':''})
	}
})
$(window).resize();
